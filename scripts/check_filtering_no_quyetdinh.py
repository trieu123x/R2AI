...
# Filter legal types (exclude Quyết định as well)
normative_types = [
    "Luật", "Bộ luật", "Nghị định", "Thông tư", "Thông tư liên tịch", 
    "Nghị quyết", "Pháp lệnh", "Hiến pháp", "Văn bản hợp nhất"
]

# We want to keep matches if it is one of our target_doc_nums OR matches the normative types
...
output = []
output.append(f"Final filtered count (no Quyết định): {len(final_filtered)}")

# Let's count how many matching contents exist and their size
final_ids = set(final_filtered['id'].tolist())
...
for file_path in content_files:
    ...
    matched_df = df_content[df_content['id'].isin(final_ids)]
    matched_count += len(matched_df)
    total_bytes += matched_df['content'].fillna('').str.encode('utf-8').apply(len).sum()

output.append(f"Final Filtered Match Count: {matched_count}")
output.append(f"Final Filtered Total Content Size: {total_bytes / (1024 * 1024):.2f} MB")

# Let's list the counts of legal types in this filtered set
output.append("\nLegal types in final filtered:")
for t, c in final_filtered['legal_type'].value_counts().items():
    output.append(f"  - {t}: {c}")

with open("final_filtering_output.txt", "w", encoding="utf-8") as f:
    f.write("\n".join(output))

print("Saved output to final_filtering_output.txt")
