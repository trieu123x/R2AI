...
print("FTS manual results:")
for idx, r in enumerate(fts_results[:5], 1):
    print(f"[{idx}] {r.chunk_id} | {r.doc_number} | {r.legal_type} | score={r.score:.4f}")

# Vector search results
vector_results = retriever.vector_search(query, top_k=15)
print("\nVector results:")
for idx, r in enumerate(vector_results[:5], 1):
    print(f"[{idx}] {r.chunk_id} | {r.doc_number} | {r.legal_type} | score={r.score:.4f}")
...
for idx, r in enumerate(hybrid_results[:10], 1):
    print(f"[{idx}] {r.chunk_id} | {r.doc_number} | {r.legal_type} | score={rrf_scores[r.chunk_id]:.4f} | source={r.source}")
